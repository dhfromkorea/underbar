...
(function() {
  ...
  _.reduce = function(collection, iterator, accumulator) {
    var noInitial = (accumulator === undefined);
  ...
  _.contains = function(list, value) {
    // Your code here
    // reduce
    // if once found, status turns into true and goes on like that.
    return _.reduce(list, function(isFound, item) {
      if (isFound) {
        return true;
      }
      return item === value;
  ...
  _.defaults = function(obj) {
    ...
    // add prop to default obj
    _.each(otherObjs, function(eachOtherObj) {
      _.each(eachOtherObj, function(value, key) {
  ...
  _.once = function(func) {
    // fundamental question was
    // has this function been called previously?
    // use the closure scope which is bound to this func only
    // therefore therer's no need to check the function name
    var storage;
    var invoked = false;
    return function() {
      if (invoked) {
      ...
      storage = func.apply(null, arguments);
      invoked = true;
      return storage;
    };
  };
  ...
  _.zip = function() {
    ...
    // a helper function to find the length of an array of the longest length
    var findMaxLength = function(arrays) {
      var arrsByLength = _.map(arrays, function(item) {
        return item.length;
      });
      return _.sortBy(arrsByLength)[arrsByLength.length - 1];
    };
    var maxArrLength = findMaxLength(arrs);

    var results = [];
    for (var i = 0; i < maxArrLength; i++) {
      var zippedArr = [];
      for (var j = 0; j < arrs.length; j++) {
        zippedArr.push(arrs[j][i]);
      }
      results.push(zippedArr);
    }
    return results;
  };
  ...
  _.flatten = function(nestedArray, result) {
    // whenever visit a primitive, push to a global object
    // initialize an array
    result = result || [];
    var itemExamined;
    for (var i = 0; i < nestedArray.length; i++) {
      // check if it's an array
      itemExamined = nestedArray[i];
      if (Array.isArray(itemExamined)) {
        _.flatten(itemExamined, result);
      } else {
        // base case
        result.push(itemExamined);
      }
    }
    return result;
  };
  ...
  _.intersection = function() {
    ...
    var args = Array.prototype.slice.call(arguments);
    var winnerCandidate = args[0];
    var challengers = args.slice(1, args.length);
    var winner = [];
    for (var i = 0; i < challengers.length; i++) {
      // filter only those which both share in common only
      var challenger = challengers[i];
      for (var j = 0; j < winnerCandidate.length; j++) {
        if (_.indexOf(challenger, winnerCandidate[j]) >= 0) {
          winner.push(winnerCandidate[j]);
        }
      }
      // check if there's a need to continue
      if (winner.length === 0) {
        return null;
      } else {
  ...
  _.difference = function(firstArray) {
    var commonList = _.intersection.apply(null, arguments);
    console.log(commonList);
    // custom helper function - differencex
    return _.reject(firstArray, function(item){
      // should fail this test to be included to remain
      return _.contains(commonList, item);
    });
  };
